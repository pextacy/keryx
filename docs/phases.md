...
- ✅ **Production LLM moat wired:** `GeminiJudge` (per-claim `supported|partial|unsupported` via Gemini structured outputs, `gemini-2.5-flash`) + `GeminiAnswerer` (Gemini answer synthesis) auto-activate when `KERYX_GEMINI_API_KEY` is set, and degrade to the deterministic offline heuristics (CI/demo default) on any API error. `/config` reports which path is live.
